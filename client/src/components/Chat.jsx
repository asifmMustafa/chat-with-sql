...
const LoadingSpinner = () => (
  <svg
    aria-hidden="true"
    class="w-4 h-4 ml-2 text-gray-200 animate-spin dark:text-slate-100 fill-slate-400"
    viewBox="0 0 100 101"
    fill="none"
  ...
  const onClick = async () => {
    ...
    try {
      ...
      const response = await fetch(
        ...
        // `http://localhost:9000/.netlify/functions/api/assistant?query=${query}`,
        {
          method: "GET",
          headers: {
  ...
  return (
    <div className="sm:mt-10 mt-5 lg:w-1/2 md:w-[70%] w-full sm:text-sm text-xs">
      <div className="px-4 py-2 bg-white rounded-lg">
        ...
        <button
          onClick={onClick}
          className="inline-flex items-center mb-2 p-2 border-[1px] border-slate-200 rounded-lg shadow-sm hover:shadow-inner"
        >
          Ask Assistant {loading && <LoadingSpinner />}
        </button>
